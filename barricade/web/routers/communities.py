...
from barricade.exceptions import AdminNotAssociatedError, AlreadyExistsError
...
def get_admin_dependency(load_relations: bool):
    async def inner(db: DatabaseDep, admin_id: int):
        result = await communities.get_admin_by_id(
            db, admin_id, load_relations=load_relations
        )
        if result is None:
            raise HTTPException(
...
async def get_all_communities(
    ...
    result = await communities.get_all_communities(
        db,
...
async def create_community(
    ...
    try:
        db_community = await communities.create_new_community(
            db, community, by=(token.user.username if token.user else "Web Token")
        )
...
async def edit_community(
    ...
    community = await communities.edit_community(
        db, community, params, by=(token.user.username if token.user else "Web Token")
    )
    await db.commit()
    return community
...
async def abandon_community(
    ...
    community = await communities.abandon_community(
        db, community.id, by=(token.user.username if token.user else "Web Token")
    )
    await db.commit()
    return community
