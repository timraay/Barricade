...
def get_player_ban_dependency(load_relations: bool):
    async def inner(
        ...
        if db_ban is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Player is not banned"
            )
        return db_ban
...
PlayerBanWithRelationsDep = Annotated[
    models.PlayerBan, Depends(get_player_ban_dependency(True))
]
...
@router.get(
    "/communities/me/bans", response_model=PaginatedResponse[schemas.PlayerBanRef]
)
