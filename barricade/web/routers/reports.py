...
def get_report_dependency(load_token: bool):
    async def inner(db: DatabaseDep, report_id: int):
        result = await reports.get_report_by_id(
            db, report_id, load_relations=load_token
        )
        if result is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Report does not exist"
            )
        return result
...
async def create_report(
    ...
    if not db_admin:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Admin does not exist"
        )
...
async def validate_submission_token(
    ...
    if not token:
        logger.warning(
            "Token %s not found. Response ID: %s", submission.data.token, submission.id
        )
        raise invalid_token_error

    if token.is_expired():
        logger.warning(
            "Token %s has expired. Response ID: %s",
...
@asynccontextmanager
async def notify_of_errors_in_dms(
    token: models.ReportToken, submission: schemas.ReportSubmission
):
    try:
        yield
        ...
        try:
            ...
            logger.exception(
                "Failed to notify %s of submission failure", token.admin_id
            )
            pass
...
async def submit_report(
    ...
    async with notify_of_errors_in_dms(token, submission):
        if token.report:
            logger.warning(
                "Token %s has already been used. Response ID: %s",
                submission.data.token,
                submission.id,
            )
            raise HTTPException(
        ...
        reasons_bitflag, reasons_custom = ReportReasonFlag.from_list(
            submission.data.reasons
        )
...
async def submit_report_edit(
    ...
    if not token.report:
        logger.warning(
            "Token %s has not been used yet. Response ID: %s",
            submission.data.token,
            submission.id,
        )
        raise HTTPException(
...
async def delete_report(
    ...
    try:
        return await reports.delete_report(
            db,
            report_id=report_id,
            by=(token.user.username if token.user else "Web Token"),
        )
...
async def create_own_report(
    ...
    token: Annotated[
        web_schemas.TokenWithHash,
        Security(
            get_active_token_of_community, scopes=Scopes.REPORT_ME_MANAGE.to_list()
        ),
    ],
):
...
@router.get(
    "/communities/me/reports/{report_id}", response_model=schemas.SafeReportWithToken
)
