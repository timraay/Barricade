...
def get_integration_dependency(integration_id: int):
    ...
    if result is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Integration does not exist"
        )
    return result
...
@router.get(
    "/integrations", response_model=PaginatedResponse[schemas.SafeIntegrationConfig]
)
...
@router.get(
    "/integrations/{integration_id}", response_model=schemas.SafeIntegrationConfig
)
...
@router.post(
    ...
    response_model=schemas.SafeIntegrationConfig,
)
...
@router.get(
    "/communities/me/integrations/{integration_id}",
    response_model=schemas.SafeIntegrationConfig,
)
...
@router.post(
    "/communities/me/integrations/{integration_id}/enable",
    response_model=schemas.SafeIntegrationConfig,
)
...
@router.post(
    "/communities/me/integrations/{integration_id}/disable",
    response_model=schemas.SafeIntegrationConfig,
)
