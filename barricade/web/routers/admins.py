...
from barricade.web.routers.communities import (
    AdminDep,
    AdminWithRelationsDep,
    CommunityDep,
)
...
async def get_all_admins(
    ...
    result = await communities.get_all_admins(
        db,
...
async def create_admin(
    ...
    try:
        db_admin = await communities.create_new_admin(
            db, admin, by=(token.user.username if token.user else "Web Token")
        )
...
async def create_admin_for_own_community(
    ...
    token: Annotated[
        web_schemas.TokenWithHash,
        Security(
            get_active_token_of_community, scopes=Scopes.COMMUNITY_ME_MANAGE.to_list()
        ),
    ],
):
