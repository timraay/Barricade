...
async def get_user_by_username_dependency(
    ...
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Web user does not exist"
        )
    return user
...
async def get_all_web_users(
    token: Annotated[
        schemas.TokenWithHash, Security(get_active_token, scopes=Scopes.STAFF.to_list())
    ],
    db: DatabaseDep,
):
...
async def update_user(
    ...
    if user.username != updated_user.username and await get_user_by_username(
        db, updated_user.username
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Username already taken"
        )
