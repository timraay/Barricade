...
if TYPE_CHECKING:
    from barricade.integrations.battlemetrics.integration import (
...
class BattlemetricsWebsocket(Websocket):
    ...
    async def setup_hook(self):
        await self.execute(
            ClientRequestType.auth, payload=self.token, is_sensitive=True
        )
    ...
    async def handle_request(self, request: Packet):
        self.logger.debug(
            "Handling websocket request #%s %s %s", request.i, request.t, request.p
        )
    ...
    async def handle_response(self, response: Packet):
        ...
        if response.t == ServerRequestType.error:
            waiter.set_exception(
                WebsocketRequestException(response.p.get("detail", response.p))  # type: ignore
            )
        else:
    ...
    async def execute(
        ...
        self.logger.info(
            ...
            request.i,
            request.t.name,
            "[********]" if is_sensitive else request.p,
        )
        ...
        try:
            ...
            self.logger.error(
                'Websocket returned error "%s" for request: %r', e, request
            )
            raise
        finally:
            # Remove waiter
