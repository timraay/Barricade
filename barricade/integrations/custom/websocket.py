...
class CustomWebsocket(Websocket):
    ...
    async def handle_request(self, request: RequestBody):
        ...
        if handler:
            ...
            self.logger.warning(
                "No handler for websocket request %s", request.request.name
            )
    ...
    async def handle_response(self, response: ResponseBody):
        self.logger.info(
            "Handling websocket response #%s %s", response.id, response.response
        )
    ...
    async def execute(
        self, request_type: ClientRequestType, payload: dict | None
    ) -> dict | None:
        ...
        # Send request
        request = RequestBody(
            id=next(self._counter), request=request_type, payload=payload
        )
