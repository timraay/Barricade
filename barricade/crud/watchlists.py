...
async def get_watchlist_by_id(
    db: AsyncSession, watchlist_id: int, load_relations: bool = False
):
...
async def create_watchlist(
    db: AsyncSession, watchlist: schemas.PlayerWatchlistCreateParams
):
    ...
    try:
        ...
        raise AlreadyExistsError("Player is already watchlisted") from None
    return db_watchlist


async def bulk_create_watchlists(
    db: AsyncSession, watchlists: list[schemas.PlayerWatchlistCreateParams]
):
    if not watchlists:
        return
    ...
    stmt = stmt.on_conflict_do_nothing(index_elements=["player_id", "community_id"])
