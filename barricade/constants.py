...
def get_env_datetime(key: str, default: datetime | None = None) -> datetime | None:
    value = os.getenv(key)
    if value:
        return datetime.fromisoformat(value).astimezone(tz=UTC)
    else:
        return default
...
WEB_DOCS_VISIBLE = os.getenv("WEB_DOCS_VISIBLE", "1").strip().lower() not in (
    "",
    "0",
    "no",
    "off",
...
T17_SUPPORT_NUM_REQUIRED_RESPONSES = get_env_int(
    "T17_SUPPORT_NUM_REQUIRED_RESPONSES", 20
)
...
T17_SUPPORT_CONFIRMATION_PROMPT_CHANCE = get_env_float(
    "T17_SUPPORT_CONFIRMATION_PROMPT_CHANCE", 0.0
)
