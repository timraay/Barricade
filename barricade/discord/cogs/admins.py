...
import discord
from discord import Interaction, RawMemberRemoveEvent, app_commands
from discord.ext import commands
from discord.utils import escape_markdown as esc_md

from barricade import schemas
from barricade.constants import DISCORD_GUILD_ID, MAX_ADMIN_LIMIT
...
class AdminsCog(commands.Cog):
    ...
    @app_commands.command(
        name="add-admin", description="Add one of your community's admins to the Bunker"
    )
    @app_commands.guilds(DISCORD_GUILD_ID)
    @app_commands.default_permissions(manage_guild=True)
    async def add_admin_to_community(
        self, interaction: Interaction, user: discord.Member
    ):
        async with session_factory() as db:
            ...
            if (
                len(await owner.owned_community.awaitable_attrs.admins)
                > MAX_ADMIN_LIMIT
            ):
                raise CustomException(
    ...
    @app_commands.command(
        name="remove-admin", description="Remove an admin's access from the Bunker"
    )
    @app_commands.guilds(DISCORD_GUILD_ID)
    @app_commands.default_permissions(manage_guild=True)
    async def remove_admin_from_community(
        self, interaction: Interaction, user: discord.Member
    ):
        async with session_factory() as db:
            # Make sure the user is a community owner
            ...
            if (
                not admin
                or not admin.community
                or admin.community_id != owner.community_id
            ):
                raise CustomException(
    ...
    @app_commands.command(
        name="transfer-ownership", description="Transfer ownership to another admin"
    )
    @app_commands.guilds(DISCORD_GUILD_ID)
    @app_commands.default_permissions(manage_guild=True)
    async def transfer_ownership_of_community(
        self, interaction: Interaction, user: discord.Member
    ):
        async with session_factory() as db:
            # Make sure the user is a community owner
            ...
            if user.id == interaction.user.id:
                raise CustomException("You can't transfer ownership to yourself!")
    ...
    @app_commands.command(
        name="leave-community", description="Remove your admin access from the Bunker"
    )
    @app_commands.guilds(DISCORD_GUILD_ID)
    @app_commands.default_permissions(manage_guild=True)
    async def leave_community_as_admin(self, interaction: Interaction):
        async with session_factory() as db:
            ...
            if not admin or not admin.community_id:
                raise CustomException(
                    "You can't leave a community without being part of one...!"
                )
