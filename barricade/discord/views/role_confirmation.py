...
class AlertsRoleConfirmationView(View):
    ...
    async def send(self, interaction: Interaction):
        if self.role:
            ...
            embed = get_question_embed(
                title="Do you want to set your alerts role to match your admin role?",
                description="Alerted admins will be able to immediately ban the player, assuming the alerts role is set up.",
            )
        else:
            embed = get_question_embed(
                title="Do you want to remove your current alerts role?",
            ...
            await interaction.response.edit_message(
                embed=get_success_embed(
                    title=(
                        ...
                        if self.role
                        else f"Set the alerts role for {db_admin.community.name} to match the admin role!"
                        if self.default
                        else f"Removed the alerts role for {db_admin.community.name}"
                    )
                ),
                view=None,
            )
