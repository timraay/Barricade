...
class ReportChannelConfirmationView(View):
    def __init__(self, channel: TextChannel | None):
        ...
        self.confirm_button = CallableButton(
            self.confirm, style=ButtonStyle.green, label="Confirm", single_use=True
        )
        self.add_item(self.confirm_button)
    ...
    async def send(self, interaction: Interaction):
        if self.channel:
            ...
            embed = get_question_embed(
                title="Do you want to remove your current report feed?",
            ...
            await interaction.response.edit_message(
                embed=get_success_embed(
                    title=(
                        ...
                        if self.channel
                        else f"Removed the report feed for {db_admin.community.name}"
                    )
                ),
                view=None,
            )
...
class ConfirmationsChannelConfirmationView(View):
    ...
    async def send(self, interaction: Interaction):
        if self.channel:
            ...
            embed = get_question_embed(
                title="Do you want to set your confirmations feed to match your reports feed?",
                description="Both will be sent to the same channel, assuming the reports feed is set up.",
            )
        else:
            embed = get_question_embed(
                title="Do you want to remove your current confirmations feed?",
            ...
            await interaction.response.edit_message(
                embed=get_success_embed(
                    title=(
                        ...
                        if self.channel
                        else f"Set the confirmations feed for {db_admin.community.name} to match the reports feed!"
                        if self.default
                        else f"Removed the confirmations feed for {db_admin.community.name}"
                    )
                ),
                view=None,
            )
...
class AlertsChannelConfirmationView(View):
    ...
    async def send(self, interaction: Interaction):
        if self.channel:
            ...
            embed = get_question_embed(
                title="Do you want to set your alerts feed to match your reports feed?",
                description="Both will be sent to the same channel, assuming the reports feed is set up.",
            )
        else:
            embed = get_question_embed(
                title="Do you want to remove your current alerts feed?",
            ...
            await interaction.response.edit_message(
                embed=get_success_embed(
                    title=(
                        ...
                        if self.channel
                        else f"Set the alerts feed for {db_admin.community.name} to match the reports feed!"
                        if self.default
                        else f"Removed the alerts feed for {db_admin.community.name}"
                    )
                ),
                view=None,
            )
