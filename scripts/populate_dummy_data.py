...
async def main():
    ...
    async with session_factory.begin() as db:
        print("--", "Begun session")
        c1 = await communities.create_new_community(
            db,
            schemas.CommunityCreateParams(
                name="Wolves of War",
                tag="(WTH)",
        ...
        c2 = await communities.create_new_community(
            db,
            schemas.CommunityCreateParams(
                name="Community 2",
                tag="[C2]",
        ...
        await communities.create_new_community(
            db,
            schemas.CommunityCreateParams(
                name="Community 3",
                tag="[C3]",
    ...
    async with session_factory() as db:
        print("--", "Begun session")
        await communities.create_new_admin(
            db,
            schemas.AdminCreateParams(
                discord_id=446731539611648001, community_id=c1.id, name="Barricadeer"
            ),
        )
        print("--", "Created admin")

        t1 = await reports.create_token(
            db,
