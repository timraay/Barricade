...
class BarricadeConnectionManager:
    ...
    async def broadcast(self, request_type: ServerRequestType, payload: dict | None):
        ...
        for ws in self.active_connections:
            ...
            except (TimeoutError, BarricadeRequestError):
...
class BarricadeWebSocket:
    ...
    async def handle_response(self, response: ResponseBody):
        ...
        if response.failed:
            ...
            waiter.set_exception(BarricadeRequestError(response_data.get("error", "")))
        else:
...
async def authorize(
    ...
    raise WebSocketException(
        code=status.WS_1008_POLICY_VIOLATION, reason="Invalid token provided"
    )
