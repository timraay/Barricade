...
down_revision: str | None = "cae0c0791b80"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

message_type = postgresql.ENUM(
    "PUBLIC", "MANAGE", "REVIEW", "T17_SUPPORT", name="reportmessagetype"
)
...
def upgrade() -> None:
    ...
    op.alter_column(
        "report_messages", "community_id", existing_type=sa.INTEGER(), nullable=True
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("report_messages_pkey", table_name="report_messages")
    op.drop_index(
        "ix_report_messages_report_id_community_id", table_name="report_messages"
    )

    op.alter_column(
        "report_messages", "community_id", existing_type=sa.INTEGER(), nullable=False
    )

    op.create_primary_key(
        "report_messages_pkey", "report_messages", ["report_id", "community_id"]
    )
    op.create_index(
        "ix_report_messages_message_id", "report_messages", ["message_id"], unique=True
    )
